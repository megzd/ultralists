...
class CreateListTest(TestCase):
    ...
    def test_empty_input_renders_home_template(self):
        response = self.client.post(reverse("create_list"), data={"text": ""})
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "lists/home.html")

    def test_empty_input_displays_error(self):
        response = self.client.post(reverse("create_list"), data={"text": ""})
        self.assertContains(response, html.escape(EMPTY_ITEM_ERROR))

    def test_empty_items_are_discarded(self):
...
class UserListTest(TestCase):
    ...
    def test_empty_input_has_is_invalid_css_class(self):
        my_list = List.objects.create()
